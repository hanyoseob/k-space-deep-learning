function [net,stats] = cnn_train_dag_img2img_cartesian(net, imdb, getBatch, varargin)
...
% opts.derOutputs = {'objective1', 1, 'objective', 1} ;
% opts.derOutputs = {'objective_img', 1e-0, 'objective_fft', 1e-0} ;
...
for epoch=start+1:opts.numEpochs
  ...
  params.train = opts.train(randperm(numel(opts.train), fix(numel(opts.train)*opts.batchSample))) ; % shuffle
  params.train = params.train(1:min(opts.epochSize, fix(numel(opts.train)*opts.batchSample)));
...
%     [inputs, means]	= params.getBatch(params.imdb, batch) ;
    ...
    if strcmp(mode, 'train')
      ...
      imgid                         = net.getVarIndex('regr_img') ;
%       fftid                         = net.getVarIndex('regr_fft') ;
      net.vars(imgid).precious      = true;
%       net.vars(fftid).precious      = true;
...
%       net.vars(fftid).precious      = true;
  ...
  %%
  if (~mod(fix((t-1)/params.batchSize), 10))
      wnd           = params.imageRange;
      wnd_ft        = [0, 15];
      
      nnfft2        = params.param.nnfft2;
      nnifft2       = params.param.nnifft2;
      
      data_img_     = ri2comp(gather(inputs{2}(:,:,:,1)));
      labels_img_	= ri2comp(gather(inputs{4}(:,:,:,1)));
      rec_img_      = ri2comp(gather(net.vars(imgid).value(:,:,:,1)));
      
      %%
      data_fft_     = nnfft2(sum(data_img_, 3));
      labels_fft_   = nnfft2(sum(labels_img_, 3));
      rec_fft_      = nnfft2(sum(rec_img_, 3));
      
      data_img_     = ssos(data_img_);
      labels_img_	= ssos(labels_img_);
      rec_img_      = ssos(rec_img_);

      if strcmp(net.mode, 'normal')
          figure(10);
      else
	      figure(100);  
      end
    ...
    subplot(231); imagesc(abs(labels_img_),	wnd);           axis image off;     title('ground truth');
    subplot(232); imagesc(abs(data_img_),	wnd);           axis image off;     title('data');
    subplot(233); imagesc(abs(rec_img_),	wnd);           axis image off;     title('recon');
    
    subplot(234); imagesc(log(abs(labels_fft_)),wnd_ft);	axis image off;     title('ground truth');
    subplot(235); imagesc(log(abs(data_fft_)),	wnd_ft);   	axis image off;     title('data');
    subplot(236); imagesc(log(abs(rec_fft_)),	wnd_ft);  	axis image off;     title('recon');
